...
export class ReturnsService {
  ...
  async createReturnRequest(ctx: TenantContext, orderId: string, reason: string) {
  ...
  async approveReturnRequest(ctx: TenantContext, id: string) {
    ...
    if (returnReq.status !== 'pending') {
      throw new BadRequestException(`Cannot approve return in status ${returnReq.status}`);
    }
  ...
  async receiveReturnItem(ctx: TenantContext, id: string) {
    ...
    if (returnReq.status !== 'approved') {
      throw new BadRequestException(`Cannot receive return in status ${returnReq.status}`);
    }
  ...
  async processRefund(ctx: TenantContext, id: string) {
    ...
    if (returnReq.status !== 'received') {
      throw new BadRequestException(`Cannot refund return in status ${returnReq.status}. Must receive items first.`);
    }
