...
export function DataTable<T extends Record<string, unknown>>({
  ...
  const controlledSort = typeof onSort === "function";
  const activeSortKey = controlledSort ? (sortBy ?? null) : (sortBy ?? internalSortKey);
  const activeSortDir = controlledSort ? (sortDir ?? "asc") : (sortDir ?? internalSortDir);
                ...
                >
                  {col.sortable ? (
                    <button
                      ...
                      aria-label={
                        activeSortKey === col.key
                          ? `Sorted by ${col.label} ${activeSortDir === "asc" ? "ascending" : "descending"}`
                          : undefined
                      }
                      onClick={() => {
                        if (controlledSort) {
