...
- [x] Forgot password (email with reset token)
- [x] Reset password (validate token, set new pw)
- [x] Change password (verify current pw)
- [x] MFA/OTP setup + verification (TOTP)
- [x] Staff invitation (create user, temp password, invite email)
- [x] Admin UI for user management (list, create, assign roles)
...
- [x] Tenant list page (plan, status, created date)
- [x] Tenant provisioning flow (tenant + domain + seed defaults)
- [x] Tenant detail with plan, status, domains, feature flags
...
- [x] Category pages (fully integrated)
- [x] Cart page
...
### ⚠️ Identified Scaffolds (Missing Logic)

The following features have database models or UI pages but are missing backend business logic:

- **Inventory & Stock Reservation:** Missing reservation algorithm and logic.
- **Returns & Refunds:** Missing logic to process returns/RMAs and issue refunds.
- **Subscriptions:** Missing recurring billing engine/workers.
- **B2B Functionality:** Missing wholesale pricing logic (CompanyProfiles, PriceLists).
- **Product Bundles:** Missing cart/catalog logic for bundled items.
- **Business/Intelligence Engines:** Empty scaffold folders.
