# Skill — Theme/Template Override Merge (exact algorithm)

This is the single implementation used by theme engine, template engine, and page layout — do not write a second version of this logic anywhere in the codebase. Import from `packages/theme-engine`.
...
## 2. Required Behavior (test these exactly — see `.agent/rules/04-testing-requirements.md` §5)

| Scenario | Expected result |
|---|---|
| `base = {a: 1, b: 2}`, `override = {}` | `resolved = {a: 1, b: 2}`, `conflicts = []` |
| `base = {a: 1, b: 2}`, `override = {b: 5}` | `resolved = {a: 1, b: 5}`, `conflicts = []` |
| `base_v2` removes key `c` that `override` still sets | `conflicts = ['c']` — surface this to the merchant/admin, never silently drop or crash |
| `override.colors = ['red']`, `base.colors = ['blue','green']` | `resolved.colors = ['red']` (arrays replace, never merge/concat) |

## 3. Usage Pattern — Theme
...
## 4. Cache Invalidation — Mandatory on Every Override Write
...
`resolveOverride()` is generic — reuse it identically for `template_base` + `template_tenant_override`, and for page-section publish/draft resolution. Do not write a parallel merge function for these; if the shape genuinely differs, extend `resolveOverride`'s generics, don't fork it.
