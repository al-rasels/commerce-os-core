...
- [x] `shipping_rules` table (type: flat_rate, weight_tier; config: JSONB)
- [x] Shipping rules CRUD API + service
- [x] Flat-rate rule: fixed price per order
- [x] Weight-tier rule: price based on total order weight
- [x] Shipping method selection at checkout
- [x] Admin UI for configuring shipping rules
- [x] Tax calculation at checkout (flat % per region placeholder)

## Acceptance Criteria

- [x] Checkout displays available shipping methods with prices
- [x] Selecting a shipping method updates order total
- [x] Admin can create/edit/delete shipping rules
- [x] Rules are tenant-scoped
- [x] Free shipping threshold option works
