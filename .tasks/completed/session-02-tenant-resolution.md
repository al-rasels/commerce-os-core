...
- [x] NestJS global middleware: parse `req.hostname` → look up `tenant_domains` → attach `tenant_id`
- [x] `TenantContext` class (request-scoped provider) with fields per multi-tenant doc §2
- [x] Seed script: create 2 test tenants + domains (`tenanta.localhost`, `tenantb.localhost`)
- [x] Prisma migration: run initial migration to create `tenants` and `tenant_domains` tables
- [x] `/api/whoami` test endpoint that returns the resolved tenant
...
- [x] `tenanta.localhost:3000/api/whoami` returns tenant A's id
- [x] `tenantb.localhost:3000/api/whoami` returns tenant B's id
- [x] Unknown hostname returns 404 before any controller logic executes
- [x] `TenantContext` is injectable in any controller and logs correctly
- [x] Unit test: verify middleware rejects unmapped domains
...
- Never resolve tenant _after_ some route logic already ran
